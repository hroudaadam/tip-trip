<template>
    <div class="d-flex justify-content-between align-items-start">
        <div class="d-flex gap-md-3 gap-2 flex-column flex-md-row align-items-start">
            ...
            <select class="form-select align-self-start w-100 w-md-50" v-on:change="switchFilter">
                <option
                    v-for="f in filters.filter(x => store.getters.isSignedIn || !x.auth)"
                    v-bind:key="f.name" 
                    :selected="f.name === state.filter"
                    :value="f.name" >
                    {{ f.text }}
                </option>
            </select>
